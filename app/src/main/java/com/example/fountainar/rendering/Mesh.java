...
public class Mesh implements Closeable {
    ...
    private final int[] VERTEX_ARRAY_ID = {0};
    private final PrimitiveMode PRIMITIVE_MODE;
    private final IndexBuffer INDEX_BUFFER;
    private final VertexBuffer[] VERTEX_BUFFERS;
    ...
    public Mesh(
        ...
        this.PRIMITIVE_MODE = primitiveMode;
        this.INDEX_BUFFER = indexBuffer;
        this.VERTEX_BUFFERS = vertexBuffers;

        try {
            GLES30.glGenVertexArrays(1, VERTEX_ARRAY_ID, 0);
            GLError.maybeThrowGLException("Failed to generate a vertex array", "glGenVertexArrays");
            GLES30.glBindVertexArray(VERTEX_ARRAY_ID[0]);
     ...
     * (location 2, vec3).
     */
    public static Mesh createFromAsset(CustomRender render, String assetFileName) throws
            IOException {
        try (InputStream inputStream = render.getAssets().open(assetFileName)) {
            ...
            VertexBuffer[] vertexBuffers = new VertexBuffer[3];

            try (VertexBuffer localBuffer = new VertexBuffer(3,
                    localCoordinates);
                 VertexBuffer textureBuffer = new VertexBuffer(2,
                         textureCoordinates);
                 VertexBuffer normalsBuffer = new VertexBuffer(3, normals)) {

                vertexBuffers[0] = localBuffer;
                vertexBuffers[1] = textureBuffer;
                vertexBuffers[2] = normalsBuffer;
            } catch (Exception e) {
                Log.e("Error vertex buffer", "Failed to create vertex buffer array");
            }
    ...
    @Override
    public void close() {
        if (VERTEX_ARRAY_ID[0] != 0) {
            GLES30.glDeleteVertexArrays(1, VERTEX_ARRAY_ID, 0);
    ...
    public void lowLevelDraw() {
        ...
        if (INDEX_BUFFER == null) {
            int numberOfVertices = VERTEX_BUFFERS[0].getNumberOfVertices();

            for (int i = 1; i < VERTEX_BUFFERS.length; ++i) {
                if (VERTEX_BUFFERS[i].getNumberOfVertices() != numberOfVertices) {
                    throw new IllegalStateException(
                            "Vertex buffers have mismatching numbers of vertices");
                }
            }

            GLES30.glDrawArrays(PRIMITIVE_MODE.GL_ES_ENUM, 0, numberOfVertices);
            GLError.maybeThrowGLException("Failed to draw vertex array object", "glDrawArrays");
        } else {
            GLES30.glDrawElements(
                    PRIMITIVE_MODE.GL_ES_ENUM, INDEX_BUFFER.getSize(), GLES30.GL_UNSIGNED_INT,
                    0);
            GLError.maybeThrowGLException(
