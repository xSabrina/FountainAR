 ...
 * calculation.
 *
 * <p>See <a href="https://google.github.io/filament/Filament.md.html#lighting/imagebasedlights">
 * </a> for a more
...
public class SpecularCubemapFilter implements Closeable {
    ...
    private static final FloatBuffer COORDS_BUFFER = ByteBuffer.allocateDirect(COORDS_BUFFER_SIZE)
    ...
    public SpecularCubemapFilter(CustomRender render, int resolution, int numberOfImportanceSamples)
        ...
        try {
            ...
            VertexBuffer coordsBuffer = null;
            try {
                coordsBuffer = new VertexBuffer(COMPONENTS_PER_VERTEX, COORDS_BUFFER);
    ...
    public void close() {
        ...
        if (RADIANCE_CUBEMAP != null) {
            RADIANCE_CUBEMAP.close();
        }
        if (LD_CUBEMAP != null) {
            LD_CUBEMAP.close();
        }
    ...
    /**
     * Generates a cache of importance sampling terms in tangent space, indexed by
     * {@code [roughnessLevel-1][sampleIndex]}.
     */
    private ImportanceSampleCacheEntry[][] generateImportanceSampleCaches() {
        ImportanceSampleCacheEntry[][] result =
                new ImportanceSampleCacheEntry[NUMBER_OF_MIPMAP_LEVELS - 1][];
